...
export default function ImportStock() {
  ...
  const canSaveDiscounts = useMemo(() => {
    if (!branchId || savingDiscounts) return false
    if (b2cDiscount === '' || b2bDiscount === '') return false
    const b2c = parseFloat(b2cDiscount)
    const b2b = parseFloat(b2bDiscount)
    return !isNaN(b2c) && !isNaN(b2b)
  ...
  const baseNameNoExt = useCallback((name) => {
    const n = name.split('/').pop() || name
    const i = n.lastIndexOf('.')
    return i > 0 ? n.slice(0, i) : n
  }, [])

  const isImagePath = useCallback((p) => {
    const n = p.toLowerCase()
    return n.endsWith('.jpg') || n.endsWith('.jpeg') || n.endsWith('.png') || n.endsWith('.webp')
  }, [])

  const extractEANFromPath = useCallback(
    (path) => {
  ...
  const uploadToCloudinary = useCallback(async (blob, publicIdBase) => {
  ...
  const onUploadImages = useCallback(
    ...
    [imageZip, branchId, show, hide, ensureEanSet, isImagePath, extractEANFromPath, uploadToCloudinary]
  )
    ...
    <div className="import-page-admin">
      ...
      <div className="import-wrap-admin">
          ...
          <form className="import-form-admin" onSubmit={(e) => e.preventDefault()}>
            <div className="excel-block">
              <div className="select-wrap">
                ...
                <select className={`audience-select ${gender ? '' : 'invalid'}`} value={gender} onChange={(e) => setGender(e.target.value)} required>
        ...
        <div className="import-card-admin">
          ...
          <div className="import-subtitle-admin">Set discount percentages for all products in this branch. These are kept separate from Excel and image uploads.</div>
